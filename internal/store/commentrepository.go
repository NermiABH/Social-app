...
func (r *CommentRepository) GetByID(c *model.Comment) error {
	q := `SELECT c.author_id, u.username, u.userpic, c.post_id, c.parent_id,
		...
		WHERE c.id = $1
		GROUP BY c.author_id, c.post_id, u.username, u.userpic, c.parent_id, c.text, c.created, c.changed`
	return r.store.db.QueryRow(q, c.ID).Scan(
		&c.AuthorID, &c.AuthorUsername, &c.AuthorUserpic, &c.PostID, &c.ParentID,
		&c.Text, &c.Created, &c.Changed, &c.Liked, &c.Disliked)
}
...
func (r *CommentRepository) ExistByID(id int) (bool, error) {
...
func (r *CommentRepository) IsOwnerByID(uID, cID int) (bool, error) {
